...
% see the LICENSE file included with this software
    ...
    %% METHODS
        ...
        %   See also controller/execute
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Here is the Finite State Machine of full navigation
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           
            % (i) If 'at goal', then switch to 'stop'. 
            if (obj.check_event('at_goal'))
                ...
                robot_moving(obj, inputs,dt);
                return;
            end
            
            % (ii) If 'unsafe', then switch to state 'avoid obstacles'.
            if obj.check_event('unsafe') % || obj.check_event('at_obstacle')
                fprintf('unsafe - avoid_obstacles\n');
                set_progress_point(obj);    % this turns out to be key. Without it, the robot cannot move out of the "room"
                obj.switch_to_state('avoid_obstacles');
                robot_moving(obj, inputs,dt);
                return;
            end
                      
            % (iii) If in state 'go to goal' and 'at obstacle', then check
            % whether the robot needs to slide left or slide right. If so,
            % set_progress_point, and switch to state 'follow wall' (with
            % inputs.direction equal to right or left depending on the
            % results of the sliding test).
            if obj.check_event('at_obstacle')
                if ~obj.check_event('progress_made')
                    [x, y, theta] = obj.state_estimate.unpack();
                    fprintf('current distance to goal = %f   obj.d_prog = %f\n', norm([x-obj.goal(1);y-obj.goal(2)]), obj.d_prog);
                    fprintf('progress not made, follow_wall\n');
                    set_progress_point(obj);
                    obj.switch_to_state ('follow_wall');
                    robot_moving(obj, inputs, dt);
                else
                    fprintf('progress made, keep avoid_obstacle\n');
                    obj.switch_to_state ('avoid_obstacles');
                    robot_moving(obj, inputs, dt);
                end
                return;
            ...
            % (iv) If in state 'follow wall', check whether progress made and
            % the robot does not need to slide slide left (or slide right
            % depending on inputs.direction). If so, switch to state 'go to
            % goal', otherwise keep 'following wall'.
            if obj.check_event('sliding_left') || obj.check_event('sliding_right')
                if obj.check_event('sliding mode, and progress_made')
                    fprintf('progress made, switch from follow_wall to go_to_goal\n');
            ...
            % (v) If in state 'obstacle cleared', go_to_goal
            if obj.check_event('obstacle_cleared')
                fprintf('obstacle cleared, go_to_goal\n');
                obj.switch_to_state ('go_to_goal');
                robot_moving(obj, inputs, dt);
            end
            %% END CODE BLOCK %%
...
%             fprintf('current_pose: (%0.3f,%0.3f,%0.3f)\n', x, y, theta);
        ...
        function robot_moving(obj, inputs, dt)
            inputs.direction = obj.fw_direction;
            outputs = obj.current_controller.execute(obj.robot, obj.state_estimate, inputs, dt);
        ...
        function rc = sliding_left(obj, state, robot)
            inputs = obj.controllers{7}.inputs;  % obj.controllers{7} = SlidingMode()
            inputs.x_g = obj.goal(1);
            inputs.y_g = obj.goal(2);
...
%                 fprintf('now sliding right\n');
        ...
        function rc = progress_made(obj, state, robot)
            ...
            if (norm([x;y] - [x_g;y_g]) < obj.d_prog - epsilon )
                rc = true;
            else
                rc = false;
            end
        ...
        function rc = unsafe(obj, state, robot)
            ...
            % Loop through and test the sensors (only the front set)
            if any(ir_distances(2:4) < obj.d_unsafe)
                    rc = true;
            end
        end
